...
from tensorflow.keras.layers import Conv1D ,AveragePooling1D,LSTM,TimeDistributed
...
## A Multi-Channel Multi-Head CNN Framework for Fault
Classification in Industrial Process


Human Activity Recognition using Multi-Head CNN
followed by LSTM 

def regression_Mutiple_head(timestep):
    dropR=0.2
    INp = keras.Input(shape=(timestep,2))
    x = Conv1D(filters=32, kernel_size=3, padding='same',strides=1)(INp)
    x = Activation(activation='relu')(x)
    x = MaxPooling1D(pool_size=2, strides=None, padding="valid")(x)
    
    x = Conv1D(filters=32, kernel_size=3, padding='valid',strides=1)(x)
    x = Activation(activation='relu')(x)
    x = MaxPooling1D(pool_size=2, strides=None, padding="valid")(x)

    x = LSTM(units =50, activation='tanh', return_sequences = True)(x)
    x = Dropout(dropR)(x)
    x = LSTM(units =8, activation='tanh', return_sequences = False)(x)
    x = Dropout(dropR)(x)
...
def Runall(trainX_, trainy_, testX_, testy_, Vax_, Vay_, epo, b,lr,n_timesteps,n_train,n_va,PATH):
    ...
    model=regression_CNNLSTM(n_timesteps)
    ...
    keras_callbacks= [EarlyStopping(monitor='val_loss',  patience=10,baseline=None, mode='auto',restore_best_weights=True),
...
for r in range(len(n_params)):
    ...
    for k in range(len(learing)):  
        save_figure_path='./CNNLSTM_{}_{}_{}_{}'.format(Ba,learing[k],ep,step)
