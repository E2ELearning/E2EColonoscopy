...
from tensorflow.keras.layers import UpSampling1D,Activation,LeakyReLU,ReLU,Dense,GlobalAveragePooling1D ,BatchNormalization,Dropout,Flatten
...
## The Performance of LSTM and BiLSTM in Forecasting Time Series [bi i sbteetr than nomal ]

## Adaptability of Financial Time Series Prediction Based on BiLSTM (Hinton [20] found that the Dropout rate of 0.2 at the input level is suitable for various tasks
# hidden layers is set to 128.



# def regression_CNNBiLSTM(timestep):
#     dropR=0.5
#     INp = keras.Input(shape=(timestep,2))
#     x = Conv1D(filters=16, kernel_size=7,padding='causal')(INp )
...
# A hybrid deep neural network based on multi-time
# window convolutional bidirectional LSTM for civil
# aircraft APU hazard identification

def regression_CNNBiLSTMArticle(timestep):
    
    dropR=0.5
    INp = keras.Input(shape=(timestep,2))
    x = Conv1D(filters=30, kernel_size=7,padding='causal')(INp )
    x = Activation(activation='relu')(x)
    
    x = Bidirectional(LSTM(30))(x)
    x = Activation(activation='tanh')(x)
    #x = Bidirectional(LSTM(30), activation='tanh')(x)
...
## A novel DAS signal recognition method based on spatiotemporal information extraction with 1DCNNs-BiLSTM network (not clear)

def regression_CNNBiLSTMArticle2(timestep):
    
    dropR=0.5
    INp = keras.Input(shape=(timestep,2))
    x = Conv1D(filters=25, kernel_size=32,padding='causal')(INp )
    x = Activation(activation='relu')(x)
    x = MaxPooling1D()(x)
    x = Conv1D(filters=25, kernel_size=64,padding='causal')(x)
    x = Activation(activation='relu')(x)
    x = MaxPooling1D()(x)
    
    x = Conv1D(filters=5, kernel_size=128,padding='causal')(x)
    x = Activation(activation='relu')(x)
    x = MaxPooling1D()(x)
    ...
    x = Conv1D(filters=25, kernel_size=5,padding='causal')(x)
    x = Activation(activation='relu')(x)
    x = MaxPooling1D()(x)
...
def regression_CNNBiLSTMArticle3(timestep):
...
def Runall(trainX_, trainy_, testX_, testy_, Vax_, Vay_, epo, b,lr,n_timesteps,n_train,n_va,PATH):
    ...
    model=regression_CNNBiLSTMArticle(n_timesteps)
    ...
    keras_callbacks= [EarlyStopping(monitor='val_loss',  patience=101,baseline=None, mode='auto',restore_best_weights=True),
...
for r in range(len(n_params)):
    ...
    for k in range(len(learing)):  
        save_figure_path='./CNNBiLSTM_{}_{}_{}_{}'.format(Ba,learing[k],ep,step)
