...
def DCNNLSTMpoeposed1(timestep):
    dropR=0.5
...
def DCNNLSTMpoeposed2(timestep):
    dropR=0.5
    INp = keras.Input(shape=(timestep,2))
    x = Conv1D(filters=8, kernel_size=5,padding='causal')(INp )
...
def DCNNLSTMpoeposed3(timestep):
    dropR=0.5
    INp = keras.Input(shape=(timestep,2))
    x = Conv1D(filters=16, kernel_size=7,padding='causal')(INp )
    x = BatchNormalization()(x)
...
#https://www.kaggle.com/code/kepure/1d-cnn-lstm
# https://github.com/julienmaitre/Basic-Daily-Activity/blob/master/CNNLSTMModels.py

# https://github.com/broadinstitute/keras-resnet/blob/master/keras_resnet/models/_time_distributed_2d.py
def CNNlstmTIME(timestep):
    dropR=0.5
    INp = keras.Input(shape=(-1,timestep,2,1))
    x = TimeDistributed(Conv1D(filters=16, kernel_size=7,padding='causal'))(INp )
...
def Runall(trainX_, trainy_, testX_, testy_, Vax_, Vay_, epo, b,lr,n_timesteps,n_train,n_va,PATH):
    ...
    model=DCNNLSTMpoeposed1(n_timesteps)
...
for r in range(len(n_params)):
    ...
    for k in range(len(learing)):  
        save_figure_path='./DCNNLSTMpoeposed12022112patien100_{}_{}_{}_{}'.format(Ba,learing[k],ep,step)
