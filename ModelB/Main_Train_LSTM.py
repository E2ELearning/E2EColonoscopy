...
def regression_LSTM(timestep):
    dropR=0.2
    INp = keras.Input(shape=(timestep,2))
    x = LSTM(128,activation='tanh', return_sequences = False)(INp)
    x = Dropout(dropR)(x)
...
# Modeling Nonlinear Electronic Components


def regression_LSTM_2(timestep):
    dropR=0.2
    INp = keras.Input(shape=(timestep,2))
    x = LSTM(3,activation='tanh', return_sequences = False)(INp)
    x = Dropout(dropR)(x)
...
def Runall(trainX_, trainy_, testX_, testy_, Vax_, Vay_, epo, b,lr,n_timesteps,n_train,n_va,PATH):
    ...
    model=regression_LSTM_2(n_timesteps)
...
for r in range(len(n_params)):
    ...
    for k in range(len(learing)):  
        save_figure_path='./LSTM2patien100_{}_{}_{}_{}'.format(Ba,learing[k],ep,step)
