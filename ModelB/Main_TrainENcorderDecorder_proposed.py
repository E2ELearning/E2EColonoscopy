...
Name_of_model ="CNNAutoencoder_Pro1"

def CNN_Autoencoder1(timestep):
    dropR=0.5
    inputs1 = keras.Input(shape=(timestep,2))
    x = Conv1D(filters=128, kernel_size=12,padding='causal')(inputs1)
    x = LeakyReLU(0.01)(x)
    x = MaxPooling1D(pool_size=2)(x)
    
    x = Conv1D(filters=64, kernel_size=8,padding='causal')(x)
    x = LeakyReLU(0.01)(x)
    x = MaxPooling1D(pool_size=2)(x)
    
    x = Conv1D(filters=32, kernel_size=5,padding='causal')(x)
    x = Conv1D(filters=16, kernel_size=5,padding='causal')(x)
    x = LeakyReLU(0.01)(x)


    decoder = Conv1D(filters=16, kernel_size=5,padding='causal')(x)
    decoder = LeakyReLU(0.01)(decoder)
    decoder =UpSampling1D(2)(decoder)
    
    decoder = Conv1D(filters=32, kernel_size=7,padding='causal')(decoder)
    decoder = Conv1D(filters=32, kernel_size=77,padding='causal')(decoder)
    decoder = LeakyReLU(0.01)(decoder)
    decoder =UpSampling1D(2)(decoder)
    
    decoder= Conv1D(filters=64, kernel_size=12,padding='causal')(decoder)
    decoder = LeakyReLU(0.01)(decoder)
    
    #decoder = GlobalAveragePooling1D()(decoder)
    decoder = Flatten()(decoder)
    decoder =Dense(50, activation='relu')(decoder)
    output_layer =Dense(2, activation='linear')(decoder)
    modelF = Model(inputs=inputs1, outputs=output_layer)
    return modelF


def CNN_Autoencoder2(timestep):
    dropR=0.5
    inputs1 = keras.Input(shape=(timestep,2))
    x = Conv1D(filters=128, kernel_size=12,padding='causal')(inputs1)
    ...
    x = Conv1D(filters=64, kernel_size=8,padding='causal')(inputs1)
    x = LeakyReLU(0.01)(x)
    x = MaxPooling1D(pool_size=2)(x)
    
    x = Conv1D(filters=32, kernel_size=5,padding='causal')(inputs1)
    x = Conv1D(filters=16, kernel_size=5,padding='causal')(inputs1)
    x = LeakyReLU(0.01)(x)
...
def Runall(trainX_, trainy_, testX_, testy_, Vax_, Vay_, epo, b,lr,n_timesteps,n_train,n_va,PATH):
    ...
    model=CNN_Autoencoder1(n_timesteps)
...
for r in range(len(n_params)):
    ...
    for k in range(len(learing)):  
        save_figure_path='./{}_{}_{}_{}_{}'.format(Name_of_model,Ba,learing[k],ep,step)
