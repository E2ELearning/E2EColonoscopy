...
def CNN_Autoencoder(timestep):
    dropR=0.5
    inputs1 = keras.Input(shape=(timestep,2))
    x = Conv1D(filters=64, kernel_size=11,padding='causal')(inputs1)
    x = LeakyReLU(0.01)(x)
    x = MaxPooling1D(pool_size=2)(x)
    
    x = Conv1D(filters=32, kernel_size=11,padding='causal')(x)
    x = LeakyReLU(0.01)(x)
    x = MaxPooling1D(pool_size=2)(x)
    
    x = Conv1D(filters=16, kernel_size=9,padding='causal')(x)
    x = LeakyReLU(0.01)(x)
    x = Conv1D(filters=1, kernel_size=7,padding='causal')(x)
    x = LeakyReLU(0.01)(x)

    

    decoder = Conv1D(filters=16, kernel_size=7,padding='causal')(x)
    decoder = LeakyReLU(0.01)(decoder)
    
    decoder = Conv1D(filters=32, kernel_size=9,padding='causal')(decoder)
    decoder = LeakyReLU(0.01)(decoder)
    

    decoder =UpSampling1D(2)(decoder)
    decoder = Conv1D(filters=64, kernel_size=11,padding='causal')(decoder)
    decoder = LeakyReLU(0.01)(decoder)
    
    decoder =UpSampling1D(2)(decoder)
    decoder= Conv1D(filters=1, kernel_size=11,padding='causal')(decoder)
    decoder = LeakyReLU(0.01)(decoder)
...
def Runall(trainX_, trainy_, testX_, testy_, Vax_, Vay_, epo, b,lr,n_timesteps,n_train,n_va,PATH):
    ...
    model=CNN_Autoencoder(n_timesteps)
...
for r in range(len(n_params)):
    ...
    for k in range(len(learing)):  
        save_figure_path='./CNNEnDecoderGlobalAveragPatien_{}_{}_{}_{}'.format(Ba,learing[k],ep,step)
