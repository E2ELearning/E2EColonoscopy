...
#from LoadandPre import loadmodel
...
##  A One-Dimensional Convolutional Neural Network (1D-CNN) Based Deep Learning System for Network Intrusion Detection [ Good for Ref Writting]
## A 1D CNN for high accuracy classification and transfer learning in motor imagery EEG-based brain-computer interface

## Batch Normalization: Accelerating Deep Network Training by Reducing Internal Covariate Shift  [suggests adding batch normalization before the activation function of the previous layer]

def regression_1dcnn(timestep):
    dropR=0.5
    INp = keras.Input(shape=(timestep,2))
    x = Conv1D(filters=32, kernel_size=20, padding='same',strides=1)(INp)
    x = BatchNormalization()(x)
    x = Activation(activation='relu')(x)
    
    x = Conv1D(filters=32, kernel_size=20, padding='valid',strides=1)(x)
    x = BatchNormalization()(x)
    x = Activation(activation='relu')(x)
    x = Dropout(dropR)(x)

    x = Conv1D(filters=32, kernel_size=6, padding='valid',strides=1)(x)
    x = Activation(activation='relu')(x)
...
def Runall(trainX_, trainy_, testX_, testy_, Vax_, Vay_, epo, b,lr,n_timesteps,n_train,n_va,PATH):
    ...
    model=regression_1dcnn(n_timesteps)
    
   
    ### One-Dimensional Convolutional Neural Network with
        # Adaptive Moment Estimation for Modelling of the Sand
        # Retention Test
    ...
    #model.compile(optimizer = Ad, loss = 'mean_squared_error', metrics = ['mae',lr_metric])
    ...
    # path1 = filepath[1:]
    # Allpath = r'/home/rc/Desktop/syfile/1DCNN_Pre' + path1
    
    # print(path1)
    # print(Allpath)
    # K=loadmodel(Allpath,Vax_,Vay_)
...
for r in range(len(n_params)):
    ...
    for k in range(len(learing)):  
        save_figure_path='./1DCNN_{}_{}_{}_{}'.format(Ba,learing[k],ep,step)
        ...
        # MSEx = mean_squared_error(Ytest[:,0], p[:,0])
        # MSEy = mean_squared_error(Ytest[:,1], p[:,1])
        # mseall=(MSEx+MSEy)/2
        # RMSEall = mseall**0.5
        # print("mseall :",mseall, "  RMSEall :",RMSEall)
